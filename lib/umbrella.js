...
module.exports = function(parentModule) {
  ...
  var injectProxy = null; // Will be set to a function used internally by umbrella
  var buildProxy = null; // Will be set to a function used internally by umbrella
  ...
  var umbrella = {
    ...
    bootstrap: function(components) {
      ...
      // Create the injectProxy and buildProxy for later use
      // Inject proxy will call injectDependencies for current umbrella object
      injectProxy = function(func, mustInvoke) {
        return utilities.injectDependencies(func, self, injectComponents, mustInvoke);
      };
      // Build proxy will call buildAndInject for current umbrella object
      buildProxy = function(path) {
        return utilities.buildAndInject(path, self, injectComponents);
      };
    ...
    buildMiddlewares: function() {
      ...
      internals.middlewares = buildProxy(this.root + constants.MIDDLEWARES_PATH);
      return this;
    },
    ...
    configure: function() {
      ...
      this.app.configure(injectProxy.bind(this, config.all, true))
        .configure('development',injectProxy.bind(this, config.dev, true))
        .configure('production', injectProxy.bind(this, config.pro, true));
    ...
    init: function(initCallback) {
      ...
      var initializers = injectProxy(umbrellaInit, true).map(function(initDescriptor) {
        ...
        return function(callback) {
          if(async) {
            // Get the asynchronous initialization wrapper with done callback
            var asyncInit = injectProxy(initializer, false);
    ...
    bootstrapModels: function() {
      ...
      var modelDirAbs = this.root + constants.MODEL_DIR;
      ...
      if(utilities.folderExists(modelDirAbs)) {
        utilities.mapAllFilesSync(modelDirAbs, function(file) {
          ...
          injectProxy(require(modelDirAbs + file), true);
        });
      }
    ...
    route: function() {
      ...
      internals.routes = buildProxy(this.root + constants.ROUTES_PATH);

      // Call routes config with dependencies
      injectProxy(umbrellaRoutes, true);
